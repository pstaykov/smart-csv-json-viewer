...
const papaparse_1 = __importDefault(__webpack_require__(3));
/**
...
function activate(context) {
    const disposable = vscode.commands.registerCommand('smartViewer.open', () => {
        ...
        const panel = vscode.window.createWebviewPanel('smartDataViewer', `Smart Viewer: ${path.basename(document.fileName)}`, vscode.ViewColumn.Beside, { enableScripts: true });
...
function deactivate() { }
/* ---------------- JSON TREE VIEW ---------------- */
function getJsonWebviewContent(data) {
...
function jsonToTree(data) {
    ...
    `).join('');
        ...
        const className = type === 'string' ? 'value-string' :
...
/* ---------------- CSV TABLE VIEW ---------------- */
function getCsvWebviewContent(csvText) {
    const parsed = papaparse_1.default.parse(csvText.trim(), { header: true });
